...
"""
db_add(table_name,data_dict,server_name)
"""
def db_add(table,data_dict,server='online_w',dbg=False):
    conn = None
    cursor=None
    db_config = {}
    insert_id=-1
...
value_list=[pid]
results = db_query(sql,value_list,'online_r')

"""
def db_query(sql,value_list=None,server='online_r',dbg=False):
    conn = None
    cursor=None
    db_config = {}
    if CONST_DB_CONFIG.has_key(server):
...
"""
def db_query_info(sql,server='online_r',dbg=False):
...
"""
def convert_results(sql,server='online_r'):
    results,colum = db_query_info(sql,server)
    colum_len = len(colum)
    results_data =[]
    if results!=None:
        for r in results:
    ...
    return colum,results_data



"""
db_update('my_table',sql)
"""
def db_update(sql,server='online_w',dbg=False):
    conn = None
    cursor=None
    db_config = {}
    if CONST_DB_CONFIG.has_key(server):
...
"""
def db_update_data(table,data_dict,server='online_w',where_dict=None):
